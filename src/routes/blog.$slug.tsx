...
export function blogPostHead(post: { title: string; excerpt: string; cover: string }, city?: { name: string } | null) {
  const suffix = city ? ` — ${city.name} | Твой3д` : " — Блог Твой3д";
  return [
...
export function BlogPostError({ message }: { message: string }) {
  return (
...
export function BlogPostPage({ post, city }: { post: any; city?: any }) {
  return (
          ...
          <div className="mt-12 space-y-8">
            {post.content.map((s: { heading?: string; body: string }, i: number) => (
              <motion.section key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5, delay: i * 0.05 }}>
                {s.heading && <h2 className="font-display text-2xl sm:text-3xl font-semibold leading-tight">{s.heading}</h2>}
