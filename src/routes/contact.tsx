  ...
  head: () => ({
    meta: [
      { title: "Контакты — Заказать 3D-печать в Москве | Твой3д" },
      {
        name: "description",
        content:
          "Заказать 3D-печать: звоните или пишите в WhatsApp и Telegram. Расчёт стоимости бесплатно. Работаем Пн–Пт 10:00–19:00. Принимаем файлы STL, STEP, OBJ.",
      },
      { property: "og:title", content: "Контакты — Заказать 3D-печать в Москве | Твой3д" },
      {
        property: "og:description",
        content:
          "Свяжитесь со студией Твой3д: телефон, email, WhatsApp, Telegram. Бесплатный расчёт стоимости 3D-печати. Ответим в течение рабочего дня.",
      },
    ],
  }),
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await submitLead({
        ...
        contact,
        message:
          [email && phone ? `Email: ${email}` : null, message].filter(Boolean).join("\n") || null,
        source: "contact_page",
      });
    } catch {
      setBusy(false);
      ...
      <section className="relative pb-20">
        <div className="mx-auto grid max-w-[1400px] gap-5 px-5 lg:grid-cols-[1.2fr_1fr]">
          <motion.form
            ...
            transition={{ duration: 0.6 }}
            onSubmit={onSubmit}
            className="relative rounded-3xl border border-foreground/10 bg-foreground/[0.015] p-6 sm:p-8"
          >
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2">
                <label className="text-[11px] uppercase tracking-[0.22em] text-foreground/45">
                  Сообщение
                </label>
                <textarea
                  name="message"
                  rows={5}
                  placeholder="Опишите задачу, прикрепите ссылку на модель..."
                  className="mt-2 w-full resize-none rounded-2xl border border-foreground/10 bg-background/60 px-4 py-3 text-sm outline-none transition-colors focus:border-[oklch(0.58_0.22_25)]/50"
                />
              </div>
            </div>
            <button
              type="submit"
              className="mt-6 group inline-flex items-center gap-2 rounded-full bg-[oklch(0.58_0.22_25)] px-7 py-3.5 text-sm font-medium text-white shadow-[0_10px_40px_-10px_rgba(219,17,37,0.6)] transition-all hover:-translate-y-0.5 hover:bg-[oklch(0.62_0.22_25)]"
            >
              ...
              <div className="rounded-3xl border border-foreground/10 bg-foreground/[0.015] p-5">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="rounded-full border border-foreground/10 bg-background/60 px-4 py-2 text-sm text-foreground/80 transition-all hover:border-[oklch(0.58_0.22_25)]/40 hover:text-foreground"
                  >
                    Telegram
                  </a>
                </div>
...
function Field({
  ...
  return (
    <div>
      ...
      <input
        {...props}
        className="mt-2 w-full rounded-2xl border border-foreground/10 bg-background/60 px-4 py-3 text-sm outline-none transition-colors focus:border-[oklch(0.58_0.22_25)]/50"
      />
    </div>
  );
}
